...
- Current patch support: Rambler adjacent-truthful shut-up behavior, Alchemist corruption-count clues with immunity, native-verified Baa deck-view reveal on death, ordered Shaman/Twin identity traces, public Dreamer two-target automation, the native-current Poet selector, and exact Druid, Confessor, Bard, Gemcrafter, Empress, Bishop, Enlightened, Medium, Knitter, Bounty Hunter, Lover, Scout, Oracle, and Hunter clue contracts
...
- **Druid:** public Druid binds managed `Librarian`; `RangedEmpath` is its obsolete predecessor. Its resettable Day picker accepts any three distinct physical characters, including self and lifecycle-hidden seats, stores references in click order, and sorts only the IDs shown in the clue. Truth uniformly names a selected registered-Outcast occurrence: Wretch and stable Spy do not count, while ordinary Doppelganger and Drunk do. Bluff is the exact complement and, when no selected Outcast exists, draws a false role from the authored non-bluffable set `{Doppelganger, Drunk, Wretch}`. Direct observations enforce exact text, references, and provenance; current Poet excludes Druid, and unmarked archive clues retain their legacy interpretation.
