...
- Current patch support: Rambler adjacent-truthful shut-up behavior, Alchemist corruption-count clues with immunity, Baa hidden-deck reveal after death, and Dreamer2 two-target automation
...
- **Dreamer2:** picks exactly two targets and returns an ambiguous role pair like `Among #X, #Y there is: RoleA or RoleB`. `next` can auto-fire Dreamer when the solver recommends two targets, while still refusing targets with unused active abilities.
