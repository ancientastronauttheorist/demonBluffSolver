...
def _generate_evil_placements(state: GameState) -> list[dict[int, str]]:
    ...
    for pos, role in state.executed_evil_roles.items():
        ...
        # Normalize to handle spaces vs underscores + case variations
        matched = False
        norm_role = _normalize_role(role)
        for i, er in enumerate(remaining_evil_roles):
            if _normalize_role(er) == norm_role:
                remaining_evil_roles.pop(i)
                matched = True
                break
        if not matched and norm_role == "puppet":
...
def _evil_role_subsets(
    ...
    for _pos, role in state.executed_evil_roles.items():
        norm_role = _normalize_role(role)
        matched_minion = False
        for i, m in enumerate(minion_pool):
            if _normalize_role(m) == norm_role:
                if bm is not None:
                    bm -= 1
                minion_pool.pop(i)
                matched_minion = True
                break
        if not matched_minion:
            for i, d in enumerate(demon_pool):
                if _normalize_role(d) == norm_role:
                    if bd is not None:
                        bd -= 1
                    demon_pool.pop(i)
                    break
