...
3. **0 scenarios = STOP. Recovery protocol:**
   ...
   - **(b) Re-screenshot** the relevant card and verify the displayed text matches what you entered. Trust the screenshot, not your memory. **Check whether the auto-entered `info_parsed` fields match the screenshot semantics** — `auto_card`/`auto_ability` can misparse speech bubbles (e.g. a Rambler silencing message like "#X shut up!" does NOT imply FT `has_evil=False`; it's just flavor text on a silencing event). If memory reader fed the solver a value inconsistent with what the screenshot shows, correct it with the manual `card ...` command.
   ...
   - **(e) "Accept the loss" ≠ "abandon the game in-app."** `game_over loss` is a TRACKING command — it records the test case and scorecard entry. It does NOT require quitting the in-app game. Before giving up on the village:
     - **Exhaust ALL unused active abilities first.** Slayer/Druid/PD/etc. can fire even when the solver is in a 0-scenario state — a successful Slayer kill or Druid check often breaks the deadlock by adding a hard-confirmed data point. Check the last non-zero `next` output for `Available abilities` and try them in order of score.
     - **Re-check every auto-entered card** for memory-reader parsing errors (see 3b). The root cause of 0 scenarios is almost always bad input, not a solver bug.
     - **Only after exhausting abilities AND verifying all entries** should you conclude you're truly stuck.
   - **(f) NEVER abandon the in-app game via pause → "Go to menu" while ANY of these are true:** HP > 0, unused active abilities remain, unflipped cards remain, memory reader is still readable. **Abandoning kills the memory reader stream** — you lose the ability to diagnose the bug post-mortem against the true game state. If HP drops to 0 from in-game damage, the game ends itself and memory reader's final state is still capturable; but if you rage-quit to menu, the process resets and you lose ground truth. **Lost asc74_v1 recoverability this way — abandoned at 5 HP with Slayer #7 unused and a misparsed FT#4 entry.** When in doubt: leave the game sitting on its turn, run `game_over loss` for tracking, and diagnose the live memory state before doing anything else in-app.
