...
9. `set_hp <hp> <wrong_exec_cost>` at game start (defaults to cost=5).
   - **Shaman games**: Shaman overwrites one eligible Villager with another Villager's role at game start; it is not a swap and is not Baker-specific. If Baker is copied, use the live clue text: chain Bakers say "I was a [original role]" and an original Baker says "I am the original Baker."
...
- Name mappings: Gambler→Gemcrafter, Imp→Baa, Baron→Chancellor,
  Illuzionist→Shaman, Cipher→Witch, plus the asset-proven internal mappings
  Skinwalker→Mutant, Marionette→Twin Minion, Mezepheles→Puppeteer, and
...
- **Shaman is a one-way overwrite, not a swap.** Native `Illuzionist.Act` runs after Plague Doctor and before Alchemist, chooses an ordered apparent-Villager source/target pair, attempts the source marker, replaces the target with the source identity, immediately fires copied Start, then attempts the target marker. Resistance can suppress either marker. The target keeps its prior statuses, resistance, and runtime data across `InitWithNoReset`; the Rust solver represents ordered endpoints plus a viable erased-role class with `ShamanTrace`, models copied-Alchemist timing, and leaves copied Baker/runtime-data composition opaque.
- **Plague Doctor uses the raw active status and a stochastic reveal.** Managed `Puzzlemaster` can target self or dead cards. Self always displays clean. Truthful Corrupted results uniformly name a registered/runtime Evil character (including Wretch/dead Evil); Bluff inverts the status and uniformly names a Good character as Evil. Ordinary Corrupted Drunk is reported Corrupted. Asc84_v2's clean generated Drunk had inherited Alchemist resistance and never gained the status; only execution bookkeeping projects Drunk clean. `next` and the autonomous loop parse the exact public result; never inject the hidden Start target from memory into live solver state.
